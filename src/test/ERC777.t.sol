// SPDX-License-Identifier: Unlicense
pragma solidity >=0.8.0;

import {DSTest} from "ds-test/test.sol";
import {Utilities} from "./utils/Utilities.sol";
import {console} from "./utils/Console.sol";
import {Vm} from "forge-std/Vm.sol";

import {MyERC777, TestERC777Recipient} from "../ERC777.sol";
import {
    IERC1820Registry
} from "@openzeppelin/contracts/utils/introspection/IERC1820Registry.sol";

contract BaseSetup is DSTest {
    Vm internal immutable _vm = Vm(HEVM_ADDRESS);

    Utilities internal _utils;
    address payable[] internal _users;
    address internal _alice;
    address internal _bob;

    MyERC777 public token;
    IERC1820Registry public erc1820Registry;
    TestERC777Recipient public recipient;
    address public deployer;
    bytes32 private constant _TOKENS_RECIPIENT_INTERFACE_HASH =
        keccak256("ERC777TokensRecipient");

    function setUp() public virtual {
        // Init ERC1820 Registry
        _vm.etch(
            address(0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24),
            bytes(
                hex""
            )
        );
        erc1820Registry = IERC1820Registry(
            0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24
        );

        recipient = new TestERC777Recipient();
        _vm.label(address(recipient), "TokenRecipient");

        token = new MyERC777();

        _utils = new Utilities();
        _users = _utils.createUsers(5);

        _alice = _users[0];
        _bob = _users[1];
    }
}

contract HandleERC777 is BaseSetup {
    uint256 internal _maxTransferAmount = 12e18;

    function setUp() public virtual override {
        BaseSetup.setUp();
    }

    function transferERC777(
        address from,
        address to,
        uint256 transferAmount,
        bytes memory userData
    ) public {
        _vm.prank(from);
        token.send(to, transferAmount, userData);
    }

    function operatorSendERC777(
        address operator,
        address from,
        address to,
        uint256 amount,
        bytes memory userData,
        bytes memory operatorData
    ) public {
        _vm.prank(operator);
        token.operatorSend(from, to, amount, userData, operatorData);
    }
}

contract SuccessTransferTest is HandleERC777 {
    uint256 internal _mintAmount = _maxTransferAmount;

    function setUp() public override {
        HandleERC777.setUp();
        token.mint(_alice, _mintAmount, "", "");
    }

    function itTransfersERC777(
        address from,
        address to,
        uint256 amount
    ) public {
        uint256 fromBalance = token.balanceOf(from);
        uint256 toBalance = token.balanceOf(to);
        transferERC777(from, to, amount, "");

        assertEqDecimal(
            token.balanceOf(from),
            fromBalance - amount,
            token.decimals()
        );
        assertEqDecimal(
            token.balanceOf(to),
            toBalance + amount,
            token.decimals()
        );
    }

    function testTransferToReceipent() public {
        uint256 t = _mintAmount;
        itTransfersERC777(_alice, _bob, t);
    }
}

contract SuccessOperatorTransferTest is HandleERC777 {
    uint256 internal _mintAmount = _maxTransferAmount;

    function setUp() public override {
        HandleERC777.setUp();
        token.mint(_alice, _mintAmount, "", "");
        _vm.prank(_alice);
        token.authorizeOperator(_bob);
    }

    function itOperatorTransfersERC777(
        address operator,
        address from,
        address to,
        uint256 amount
    ) public {
        uint256 fromBalance = token.balanceOf(from);
        uint256 toBalance = token.balanceOf(to);
        operatorSendERC777(operator, from, to, amount, "", "");

        assertEqDecimal(
            token.balanceOf(from),
            fromBalance - amount,
            token.decimals()
        );
        assertEqDecimal(
            token.balanceOf(to),
            toBalance + amount,
            token.decimals()
        );
    }

    function testOperatorTransferToReceipent() public {
        uint256 t = _mintAmount;
        itOperatorTransfersERC777(_bob, _alice, _bob, t);
    }
}

contract HookTest is HandleERC777 {
    uint256 internal _mintAmount = _maxTransferAmount;

    function setUp() public override {
        HandleERC777.setUp();
        token.mint(_alice, _mintAmount, "", "");
    }

    function itCallsTokensReceivedHook(
        address from,
        address to,
        uint256 amount
    ) public {
        uint256 initialReceivedTokens = recipient.receivedTokens();
        transferERC777(from, to, amount, "0x1234");

        assertEqDecimal(
            recipient.receivedTokens(),
            initialReceivedTokens + amount,
            token.decimals()
        );
        assertEqDecimal(
            recipient.lastReceivedAmount(),
            amount,
            token.decimals()
        );
        assertEq(recipient.lastOperator(), from);
        assertEq(recipient.lastSender(), from);
    }

    function testTokensReceivedHookCalled() public {
        uint256 t = _maxTransferAmount;
        itCallsTokensReceivedHook(_alice, address(recipient), t);
    }
}

contract RevertOperatorTransferTest is HandleERC777 {
    uint256 internal _mintAmount = _maxTransferAmount;

    function setUp() public override {
        HandleERC777.setUp();
        token.mint(_alice, _mintAmount, "", "");
    }

    function itCannotOperatorTransferERC777(
        address operator,
        address from,
        address to,
        uint256 amount
    ) public {
        _vm.expectRevert("ERC777: caller is not an operator for holder");
        operatorSendERC777(operator, from, to, amount, "", "");
    }

    function testCannotOperatorTransferWithoutAuthorization() public {
        uint256 t = _maxTransferAmount;
        itCannotOperatorTransferERC777(_bob, _alice, _bob, t);
    }
}
